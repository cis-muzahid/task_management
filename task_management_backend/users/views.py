...
class UpdateAlertTimeView(APIView):
    """
    post:
    ...
    def post(self, request, *args, **kwargs):
        user = request.user
        new_alert_time = request.data.get('default_alert_time')

        if new_alert_time is None:
            return Response({"default_alert_time": "This field is required."}, status=status.HTTP_400_BAD_REQUEST)

        if not isinstance(new_alert_time, int) or new_alert_time <= 0:
            return Response({"default_alert_time": "Must be a positive integer."}, status=status.HTTP_400_BAD_REQUEST)

        user.default_alert_time = new_alert_time
        user.save()
